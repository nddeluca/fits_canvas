...
(function() {
  ...
  FitsDisplay = (function(_super) {
    ...
    FitsDisplay.prototype.processImage = function() {
      var invertCoeff, x, x_orig, y, yScaleCoeff, _i, _j, _ref, _ref1;
      ...
      invertCoeff = (this.fitsHeight - 1) * this.fitsWidth;
      yScaleCoeff = this.scaleRatio * this.fitsWidth;
      for (x = _i = 0, _ref = this.width - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; x = 0 <= _ref ? ++_i : --_i) {
        x_orig = ~~(x * this.scaleRatio);
        for (y = _j = 0, _ref1 = this.height - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; y = 0 <= _ref1 ? ++_j : --_j) {
          this.displayView32[(this.width * y) + x] = this.colorView32[(invertCoeff - (~~(y * yScaleCoeff))) + x_orig];
        }
      }
    };
